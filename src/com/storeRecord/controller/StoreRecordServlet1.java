...
public class StoreRecordServlet1 extends HttpServlet {
		...
		if ("getOne_For_Display".equals(action)) {
			...
			try {
				...
				if (!errorMsgs.isEmpty()) {
					RequestDispatcher failureView = req.getRequestDispatcher("/back-end/storeRecord/select_page.jsp");
					failureView.forward(req, res);
					return;
				}

				req.setAttribute("storeRecordVO", storeRecordVO);
				RequestDispatcher succesView = req.getRequestDispatcher("/back-end/storeRecord/listOneStoreRecordByPk.jsp");
				succesView.forward(req, res);

			} catch (RuntimeException e) {
				errorMsgs.add("無法取得資料" + e.getMessage());
				RequestDispatcher failureView = req.getRequestDispatcher("/back-end/storeRecord/selec_page.jsp");
				failureView.forward(req, res);
			}
		}
...
//				req.setAttribute("sumCount", sumCount);
				...
				RequestDispatcher succesView = req
						.getRequestDispatcher("/back-end/storeRecord/listOneStoreRecordByMem.jsp");
				succesView.forward(req, res);
