		...
		if("INSERT_F0R_GET_ONES_ALL".equals(action)) {
			...
			/**************step2.開始計算原總額並新增資料並listOnseAll*****************/
			 ...
			 for(int i=0;i<list.size();i++) {
				 ...
				 sumOrigin+=activityTokenVO_forSum.getTokenAmount();
			 }
			 //若成功領取後總金額=舊總額+本次領取的代幣
			 sumOrigin=sumOrigin+tokenAmount;
			 //=========開始新增同時將估計的總金額sum存進會員表格中	=========== 
			 try {
				 activityTokenService.addActivityToken(activityTokenVO,sumOrigin);
			  }catch(Exception e) {
